...
fun createSnailTree(snail: String): SnailNode? {
...
fun explodeSnail(snail: SnailNode, root: SnailNode, depth: Int): Boolean {
...
fun leavesInOrder(root: SnailNode): List<SnailNode> {
    ...
    for (i in trees.indices) {
        for (j in trees.indices) {
            if (i != j) {
                ...
                maxMag = java.lang.Long.max(mag, maxMag)
                println(maxMag)
            }
        }
    }
